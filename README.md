...
- You want code verification inside the agent's session: the same rules your pipeline enforces applied to new code in seconds.
...
- Use of a supported programming language. Agentic Analysis is available for Java, Python, JavaScript/TypeScript, CSS, HTML, XML, C#, C++ and VB.NET. Secrets detection, Docker, Kubernetes, and Terraform are also supported. Check the [official docs](https://docs.sonarsource.com/sonarqube-cloud/analyzing-source-code/agentic-analysis) for the latest list.
...
The config above does not set `SONARQUBE_TOOLSETS` as the SonarQube MCP Server enables many toolsets by default (including Agentic Analysis). If you ever want to restrict the server to a subset of these tools, set `SONARQUBE_TOOLSETS` explicitly to a comma-separated list.
...
The follow-up analysis came back clean: zero remaining findings on either modified file. The full loop—from code generation through analysis, remediation, and verification—ran in a single Copilot CLI session with no user intervention after the initial prompt.
